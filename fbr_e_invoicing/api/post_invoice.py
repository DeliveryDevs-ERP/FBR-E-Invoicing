...
def post(sales_invoice_name: str):
    ...
    # --- Party helpers ---
    seller_tax_id = (
        frappe.db.get_value("Company", doc.customer, "tax_id") if doc.customer else None
    )
    seller_name = (
        frappe.db.get_value("Company", doc.customer, "name") if doc.customer else None
    )
    ...
    for row in doc.items or []:
        ...
        value_excl_st = flt(
            row.rate
        )  # as requested: use unit rate as valueSalesExcludingST
