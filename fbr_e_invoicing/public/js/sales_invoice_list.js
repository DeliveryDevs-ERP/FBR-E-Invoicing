frappe.listview_settings["Sales Invoice"] = {
    onload(listview) {
        ...
        listview.page.add_action_item(__("Post to PRA"), async () => {
            const selected_items = listview.get_checked_items() || [];
            if (!selected_items.length) {
                frappe.msgprint(__("Please select Sales Invoices first."));
                return;
            ...
            try {
                const r = await frappe.call({
                    method: "fbr_e_invoicing.api.pra_submission.bulk_submit_invoices",
                    args: {docnames},
                    freeze: true,
                    freeze_message: __("Queuing Sales Invoices for PRA submission..."),
                });
                ...
                const already_submitted_invoices =
                    ...
                    result_rows.already_submitted_skipped ||
                    [];
                const wrong_tax_category_invoices =
                    result.wrong_tax_category_invoices ||
                    result_rows.wrong_tax_category_skipped ||
                    [];
                const raw_failed_invoices =
                    result.failed_invoices || result_rows.failed_to_queue || [];
                const failed_invoices = raw_failed_invoices.map((row) => {
                ...
                const queue_route = result.queue_route || "/app/pra-queue";

                let message = __(
                    "{0} invoice(s) have been queued for PRA submission. (Note: Invoices in Draft or Cancelled status are not posted to PRA.)",
                    [queued_count]
                );
                ...
                message +=
                    "<br><br>" +
                    __(
                        "To check the status, go to <a href=\"{0}\">PRA Queue logs</a>.",
                        [queue_route]
                    );
                ...
                if (already_submitted_invoices.length) {
                    message +=
                        "<br><br>" +
                        __(
                            "The following invoices were skipped because they are already submitted to PRA:"
                        );
                    message += "<br>" + already_submitted_invoices.join("<br>");
                }

                if (wrong_tax_category_invoices.length) {
                    message +=
                        "<br><br>" +
                        __(
                            "Tax category should be Punjab for PRA posting. The following invoices were skipped (see their ePRA tab):"
                        );
                    message += "<br>" + wrong_tax_category_invoices.join("<br>");
                }
